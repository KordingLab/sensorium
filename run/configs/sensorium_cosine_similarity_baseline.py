...
cls_loss = [
    dict(type='TorchLoss', loss_name='CrossEntropyLoss', loss_weight=1.0)
]
model = dict(
...
size = (32, 64)
albu_train_transforms = [dict(type='Resize', height=32, width=64, p=1.0)]
train_pipeline = [
...
optimization = dict(
    ...
    max_iters=200,
    optimizer=dict(
        type='AdamW', lr=0.01, betas=(0.9, 0.999), weight_decay=0.0005),
    scheduler=dict(type='CosineAnnealing', interval='step', min_lr=0.0))
base_name = 'sensorium_cosine_similarity_baseline.py'
